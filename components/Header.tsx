...
const Header: React.FC = () => {
  ...
  const getTitle = () => {
    if (location.pathname.startsWith('/dashboard')) return 'Acme Corp Dashboard';
    if (location.pathname.startsWith('/alerts')) return 'Alert Monitoring Center';
    if (location.pathname.startsWith('/events')) return 'Security Event Timeline';
    if (location.pathname.startsWith('/reports')) return 'Reports & Analytics';
    if (location.pathname.startsWith('/configuration')) return 'System Configuration';
    if (location.pathname.startsWith('/health')) return 'System Health Status';
    ...
    <header className="z-10 flex h-16 w-full shrink-0 items-center justify-between border-b border-border-dark bg-background-dark/80 backdrop-blur-md px-6 lg:px-8">
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-white text-lg font-semibold tracking-tight">{getTitle()}</h2>
        </div>
      </div>
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="hidden sm:flex items-center gap-2 rounded-full border border-success/20 bg-success/10 px-3 py-1">
          ...
          <span className="text-xs font-semibold text-success tracking-wide uppercase">System Secure</span>
        </div>
